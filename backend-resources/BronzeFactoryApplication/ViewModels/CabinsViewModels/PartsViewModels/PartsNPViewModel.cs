...
{
    public partial class PartsNPViewModel : PartsViewModel<CabinNPParts>
    {
        ...
        public GlassStrip? CloseStrip
        {
            ...
            set
            {
                if (partsListObject is not null && partsListObject.CloseStrip?.Code != value?.Code)
                {
                    ...
                    partsListObject.CloseStrip = value?.GetCloneWithSpotDefaultQuantity(PartSpot.CloseProfile, Identifier, repo);
        ...
        public IEnumerable<ProfileHinge> SelectableWallHinges
        {
            get
            {
        ...
        {
            get => partsListObject?.MiddleHinge;
            set
            {
                if (partsListObject is not null && partsListObject.MiddleHinge?.Code != value?.Code)
                {
                    var oldHinge = partsListObject.MiddleHinge;
                    partsListObject.MiddleHinge = value?.GetCloneWithSpotDefaultQuantity(PartSpot.MiddleHinge, Identifier, repo);
                    OnPropertyChanged(nameof(MiddleHinge));
                    RaisePartChanged(PartSpot.MiddleHinge, partsListObject.MiddleHinge, oldHinge);
                }
            }
        }
        public CabinPart? DefaultMiddleHinge => repo.GetDefaultPart(Identifier, PartSpot.MiddleHinge);
        public IEnumerable<CabinPart> SelectableMiddleHinges
        {
            get
            {
                var selectables = repo.GetSpotValids(Identifier, PartSpot.MiddleHinge).Select(c => repo.GetPart(c, Identifier)).ToList();
                if (partsNotPresentInValids.TryGetValue(PartSpot.MiddleHinge, out CabinPart? part)
                    && part is not null
        ...
        public PartsNPViewModel(ICabinMemoryRepository repo) : base(repo)
        {
            this.repo = repo;
        }
        ...
        public override void SetNewPartsList(CabinPartsList partsList, CabinIdentifier identifier)
        {
            ...
            partsListObject = partsList as CabinNPParts ?? throw new InvalidOperationException($"Provided Parts where of type {partsList.GetType().Name} -- and not of the expected type : {nameof(CabinNPParts)}");
