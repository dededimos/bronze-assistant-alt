...
{
    public partial class PartsVSViewModel : PartsViewModel<CabinVSParts>
    {
        ...
        {
            //Return the Close profile if any otherwise the Strip if Any otherwise null for nothing
            get
            {
        ...
        public PartsVSViewModel(ICabinMemoryRepository repo):base(repo)
        {
            this.repo = repo;
        }
        ...
        public override void SetNewPartsList(CabinPartsList partsList, CabinIdentifier identifier)
        {
            ...
            partsListObject = partsList as CabinVSParts ?? throw new InvalidOperationException($"Provided Parts where of type {partsList.GetType().Name} -- and not of the expected type : {nameof(CabinVSParts)}");
