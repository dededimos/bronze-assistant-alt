...
{
    public partial class PartsWSViewModel : PartsViewModel<CabinWSParts>
    {
        private readonly ICabinMemoryRepository repo;

        public Profile? WallFixer
        {
            get => partsListObject?.WallFixer;
            set
            {
                if (partsListObject is not null && partsListObject.WallFixer?.Code != value?.Code)
                {
                    var oldWallFixer = partsListObject.WallFixer;
                    partsListObject.WallFixer = value?.GetCloneWithSpotDefaultQuantity(PartSpot.WallSide1, Identifier, repo);
                    OnPropertyChanged(nameof(WallFixer));
                    RaisePartChanged(PartSpot.WallSide1, partsListObject.WallFixer, oldWallFixer);
                }
            }
        }
        ...
        public PartsWSViewModel(ICabinMemoryRepository repo):base(repo)
        {
            this.repo = repo;
        }
        ...
        public override void SetNewPartsList(CabinPartsList partsList, CabinIdentifier identifier)
        {
            ...
            partsListObject = partsList as CabinWSParts ?? throw new InvalidOperationException($"Provided Parts where of type {partsList.GetType().Name} -- and not of the expected type : {nameof(CabinWSParts)}");
