...
{
    public class EllipseMirrorPriceRule : IPricingRule
    {
        /// <summary>
        /// The Premium Added to the Basic Price of a Capsule Mirror
        /// </summary>
        private decimal addedPremiumPerSQM = 0;
        /// <summary>
        /// The Minimum Premium Added to a Capsule Mirror if the SQM premium is lower than this
        /// </summary>
        private decimal minimumAddedPremium = 0;

        public string RuleName { get => nameof(EllipseMirrorPriceRule); }
        ...
        public EllipseMirrorPriceRule(decimal addedPremiumPerSQM, decimal minimumAddedPremium)
        {
        ...
        public void ApplyRule(IPriceable product)
        {
            try
            {
                ...
                if (mirrorProduct.MirrorProperties.Shape is MirrorShape.Ellipse)
                {
                ...
                {
                    throw new NotSupportedException("MirrorShape is Not an Ellipse - Cannot Apply Rule");
                }
            }
            catch (Exception ex)
            {
        ...
        public bool IsApplicable(IPriceable product)
        {
            if (product is PriceableMirror m &&
                m.MirrorProperties.Shape is MirrorShape.Ellipse &&
